...
> Terminal/IDE ที่เปิดอยู่ก่อนต้อง restart เพื่อรับ env var ใหม่
...
- `fetch_json()` reads max 1MB, 2s timeout, localhost only
...
|---------|---------|---------------|
| Set env var | `winreg` (HKCU\Environment) | append to `~/.zshrc` / `~/.bashrc` |
